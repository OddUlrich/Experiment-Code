...
function enableButtons () {
	...
	// Update the button label now that the button is active
	btn.value="Click me for a periodic greeting";

	// Set the onclick action for the forth button
	btn = document.getElementById("input_greet_by_weather");
	btn.onclick=function(){greetByWeather();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a weather greeting";
}
...
function greetByPeriod () {
	// Get the name and period from the name_field element and period_field element
	var name = document.getElementById("name_field").value;
...
/*
 * Execute a request to the sayHelloByWeather() endpoints function.
 * Illustrates calling an endpoints function that takes an argument.
 */
function greetByWeather () {
	// Get the weather from the weather_field element
	var weather = document.getElementById("weather_field").value;
	
	// Call the sayHelloByWeather() function.
	// It takes one argument "weather"
	// On success, pass the response to sayHelloCallback()
	var request = gapi.client.helloworldendpoints.sayHelloByWeather({'weather': weather});
	request.execute(sayHelloCallback);
}
